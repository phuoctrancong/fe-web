  ...
  useEffect(() => {
    setTotalProduct(
      getFromLocal("cart")?.reduce(
        (total, item) => total + item.item.currentQuantity,
        0
      )
    );
    setTotalPrice(
      getFromLocal("cart").reduce(
        (total, item) =>
          item?.product?.salePrice
            ? total + item?.product?.salePrice * item.item.currentQuantity
            : total + item?.product?.price * item.item.currentQuantity,
        0
      )
    );
